...
namespace cdcchain {
    namespace consensus {
        ...
        struct MemoData
        {
             ...
             */
            fc::array<char, CDC_BLOCKCHAIN_MAX_MEMO_SIZE>     message;
        ...
        struct ExtendedMemoData
        {
            ...
            fc::array<char, CDC_BLOCKCHAIN_MAX_MEMO_SIZE>      message;
            fc::enum_type<uint8_t, MemoFlagsEnum>            memo_flags;
            fc::array<char, CDC_BLOCKCHAIN_EXTENDED_MEMO_SIZE> extra_memo;
        };
...
namespace fc {
    void to_variant(const cdcchain::consensus::WithdrawCondition& var, variant& vo);
    void from_variant(const variant& var, cdcchain::consensus::WithdrawCondition& vo);
    void to_variant(const cdcchain::consensus::MemoData& var, variant& vo);
    void from_variant(const variant& var, cdcchain::consensus::MemoData& vo);
}

FC_REFLECT_ENUM(cdcchain::consensus::WithdrawConditionTypes,
    (withdraw_null_type)
    (withdraw_signature_type)
    (withdraw_multisig_type)
    (withdraw_escrow_type)
    )
    FC_REFLECT_ENUM(cdcchain::consensus::WithdrawBalanceTypes,
    (withdraw_common_type)
    (withdraw_contract_type)
    (withdraw_margin_type)
    )
    FC_REFLECT(cdcchain::consensus::WithdrawCondition,
    (asset_id)
    (slate_id)
    (type)
    (balance_type)
    ...
    FC_REFLECT(cdcchain::consensus::TransferMemo,
    (one_time_key)
    (encrypted_memo_data)
    )
    FC_REFLECT_ENUM(cdcchain::consensus::MemoFlagsEnum,
    (from_memo)
    (to_memo)
    )
    FC_REFLECT(cdcchain::consensus::MemoData,
    (from)
    (from_signature)
    (message)
    (memo_flags)
    ...
    FC_REFLECT(cdcchain::consensus::ExtendedMemoData,
    (from)
    (from_signature)
    (message)
    (memo_flags)
    ...
    FC_REFLECT_DERIVED(cdcchain::consensus::MemoStatus,
    (cdcchain::consensus::MemoData),
    (has_valid_signature)
    (owner_private_key)
    )
    FC_REFLECT(cdcchain::consensus::WithdrawWithSignature,
    (owner)
    (memo)
    )

    FC_REFLECT(cdcchain::consensus::WithdrawWithMultisig,
    (required)
    (owners)
    (memo)
    )
    FC_REFLECT(cdcchain::consensus::withdraw_with_escrow,
    (sender)
    (receiver)
    (escrow)
    (agreement_digest)
