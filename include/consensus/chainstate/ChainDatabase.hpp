    ...
    namespace consensus {
        ...
        public:
            ...
            */
			...
			bool				store_account_entries_for_simulator(const vector<cdcchain::wallet::WalletAccountEntry>& vec_account_entry);
            /**
            ...
            */
            ...
            /**  Fetch CDC input balance in blocks that block_num of them are lower of the given block_num
            *  or equal to
            *
            * @param  block_num  uint32_t
            *
            * @return vector<CdcTrxidBalance>
            */
            vector<CdcTrxidBalance> fetch_cdc_input_balance(const uint32_t &  block_num);

            /**  Insert the CDC transaction balance to db _cdc_input_balance_entry
            *
            * @param  cdc_account  string
            * @param  cdc_balance_entry  CdcTrxidBalance
            *
            * @return void
            */
            void transaction_insert_to_cdc_balance(const string & cdc_account, const CdcTrxidBalance & cdc_balance_entry);

            /** Erase the CDC transaction balance from db _cdc_input_balance_entry
            *
            * @param  cdc_account  string
            * @param  cdc_balance_entry  CdcTrxidBalance
            *
            * @return void
            */
            void transaction_erase_from_cdc_balance(const string & cdc_account, const CdcTrxidBalance & cdc_balance_entry);
            vector<CdcTrxidBalance> fetch_cdc_full_entry(const uint32_t& block_num, const uint32_t& last_scan_block_num);
            //Must handle account before you use this function
            void transaction_insert_to_cdc_full_entry(const string& cdc_accout, const CdcTrxidBalance& cdc_balance_entry);
            //Must handle account before you use this function
            void transaction_erase_from_cdc_full_entry(const string& cdc_accout, const CdcTrxidBalance& cdc_balance_entry);
...
FC_REFLECT(cdcchain::consensus::BlockForkData, (next_blocks)(is_linked)(is_valid)(invalid_reason)(is_included)(is_known))
FC_REFLECT(cdcchain::consensus::ForkEntry, (block_id)(signing_delegate)(transaction_count)(latency)(size)(timestamp)(is_valid)(invalid_reason)(is_current_fork))
