...
FC_REFLECT_ENUM(cdcchain::consensus::AccountType,
    (titan_account)
    (public_account)
    (multisig_account)
    )
    FC_REFLECT(cdcchain::consensus::MultisigMetaInfo,
    (required)
    (owners)
    )
    FC_REFLECT(cdcchain::consensus::AccountMetaInfo,
    (type)
    (data)
    )
    FC_REFLECT(cdcchain::consensus::DelegateStats,
    (votes_for)
    (pay_rate)
    (signing_key_history)
    ...
    FC_REFLECT(cdcchain::consensus::VoteDel,
    (votes)
    (delegate_id)
    )
    FC_REFLECT(cdcchain::consensus::AccountEntry,
    (id)
    (name)
    (public_data)
    (owner_key)
    ...
    FC_REFLECT(cdcchain::consensus::DelegatePaySalary, (total_balance)(pay_balance))
