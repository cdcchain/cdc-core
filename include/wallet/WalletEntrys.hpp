...
namespace cdcchain {
    namespace wallet {
        ...
        struct AccountData : public cdcchain::consensus::AccountEntry
        {
...
{
    template<>
    struct hash < cdcchain::wallet::ScriptRelationKey >
    {
        size_t operator()(const cdcchain::wallet::ScriptRelationKey& s)const
        {
...
FC_REFLECT_ENUM(cdcchain::wallet::WalletEntryTypeEnum,
    ...
    FC_REFLECT_ENUM(cdcchain::wallet::PropertyEnum,
    (version)
    (next_entry_number)
    (next_child_key_index)
    (automatic_backups)
...
(last_scanned_block_number_for_ubcore)
    ...
    FC_REFLECT_DERIVED(cdcchain::wallet::AccountData, (cdcchain::consensus::AccountEntry),
    (is_my_account)
    (approved)
    (is_favorite)
    ...
    FC_REFLECT_DERIVED(cdcchain::wallet::AccountAddressData, (cdcchain::wallet::AccountData),
    (owner_address)
    (active_address)
    )

    FC_REFLECT(cdcchain::wallet::MasterKey,
    (encrypted_key)
    (checksum)
    )

    FC_REFLECT(cdcchain::wallet::KeyData,
    (account_address)
    (public_key)
    ...
    FC_REFLECT_ENUM(cdcchain::wallet::ContactData::ContactTypeEnum,
    (account_name)
    (public_key)
    (address)
    (btc_address)
    ...
    FC_REFLECT(cdcchain::wallet::ContactData,
    (contact_type)
    (data)
    (label)
    (favorite)
    ...
    FC_REFLECT(cdcchain::wallet::TransactionData,
    (entry_id)
    (block_num)
    (is_virtual)
    (is_confirmed)
    ...
    FC_REFLECT(cdcchain::wallet::LedgerEntry,
    (from_account)
    (to_account)
    ...
    FC_REFLECT(cdcchain::wallet::TransactionLedgerEntry,
    (id)
    (block_num)
    (block_id)
    (timestamp)
    ...
    FC_REFLECT_DERIVED(cdcchain::wallet::PrettyTransactionExperimental, (cdcchain::wallet::TransactionLedgerEntry),
    (inputs)
    (outputs)
    (balances)
    (notes)
    ...
    FC_REFLECT(cdcchain::wallet::Setting,
    (name)
    (value)
    )

    FC_REFLECT(cdcchain::wallet::GenericWalletEntry,
    (type)
    (data)
    )

    FC_REFLECT(cdcchain::wallet::ScriptEntry,
    (id)
    (enable)
    (code)
    (description)
    ...
    FC_REFLECT(cdcchain::wallet::ScriptEntryPrintable,
    (id)
    (enable)
    (code_printable)
    (description)
    ...
    FC_REFLECT(cdcchain::wallet::ScriptRelationKey,
    (contract_id)
    (event_type)
    )
...
namespace fc {
    ...
    template<typename T, cdcchain::wallet::WalletEntryTypeEnum N>
    struct get_typename < cdcchain::wallet::WalletEntry<T, N> >
    {
    ...
    template<typename Type, cdcchain::wallet::WalletEntryTypeEnum N>
    struct reflector < cdcchain::wallet::WalletEntry<Type, N> >
    {
        typedef cdcchain::wallet::WalletEntry<Type, N>  type;
        ...
        {
            {
                typedef decltype(((cdcchain::wallet::BaseEntry<N>*)nullptr)->wallet_entry_index) member_type;
                visitor.TEMPLATE operator() < member_type, cdcchain::wallet::BaseEntry<N>, &cdcchain::wallet::BaseEntry<N>::wallet_entry_index > ("index");
            }
