...
namespace cdcchain {
    namespace wallet {
        ...
        struct TransactionBuilder {
            ...
            std::unordered_set<cdcchain::consensus::Address>                                      required_signatures;

            ///Map of <owning account address, asset ID> to that account's balance in that asset ID
            std::map<std::pair<cdcchain::consensus::Address, AssetIdType>, ShareType>          outstanding_balances;

            ///Map of account address to key owning that account's market transactions
            std::map<cdcchain::consensus::Address, PublicKeyType>                               order_keys;
             ...
             */
            PublicKeyType order_key_for_account(const cdcchain::consensus::Address& account_address, const string& account_name);
             ...
             */
            /// @{
            void deduct_balance(const cdcchain::consensus::Address& account_to_charge, const cdcchain::consensus::Asset& amount)
            {
            ...
            void credit_balance(const cdcchain::consensus::Address& account_to_credit, const cdcchain::consensus::Asset& amount)
            {
...
} //namespace cdcchain::wallet

FC_REFLECT_ENUM(cdcchain::wallet::VoteStrategy,
    (vote_none)
    (vote_all)
    (vote_random)
    (vote_recommended)
    )
    FC_REFLECT(cdcchain::wallet::TransactionBuilder, (transaction_entry)(required_signatures)(outstanding_balances))
