...
namespace cdcchain {
    namespace wallet {
        ...
        namespace detail {
            ...
            void WalletImpl::state_changed(const PendingChainStatePtr& state)
            {
                ...
                if (_blockchain->get_head_block_num() < last_unlocked_scanned_number)
                {
                    ...
                    self->set_last_scanned_block_number_for_cdc(_blockchain->get_head_block_num());
                }
            }
            ...
            {
                if (!self->is_open() || !self->is_unlocked()) return;
                auto ntptime = cdcchain::consensus::ntp_time();
				fc::time_point t = (ntptime.valid() ? *ntp_time() : fc::time_point::now());
                if ((t - _blockchain->get_head_block_timestamp()) <= fc::seconds(CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            */
                ...
                try {
                    ...
                    try
                    {
                        ...
                        const auto update_progress = [=](const uint32_t count)
                        {
                            ...
                            if (count % 10000 == 0) ulog("Scanning ${p} done...", ("p", cdcchain::console::pretty_percent(_scan_progress, 1)));
                        };
                        ...
                        while (current_block_num <= head_block_num && count < limit && !_scan_in_progress.canceled())
                        {
                            ...
                            self->set_last_scanned_block_number_for_cdc(prev_block_num);
                            ++current_block_num;
            ...
            void WalletImpl::upgrade_version()
            {
                ...
                if (current_version > CDC_WALLET_VERSION)
                {
                    FC_THROW_EXCEPTION(unsupported_version, "Wallet version newer than client supports!",
                        ("wallet_version", current_version)("supported_version", CDC_WALLET_VERSION));
                }
                else if (current_version == CDC_WALLET_VERSION)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        self->set_transaction_fee(Asset(CDC_WALLET_DEFAULT_TRANSACTION_FEE));
                        self->set_transaction_imessage_fee_coe(CDC_BLOCKCHAIN_MIN_MESSAGE_FEE_COE);
                        self->set_transaction_imessage_soft_max_length(CDC_BLOCKCHAIN_MAX_SOFT_MAX_MESSAGE_SIZE);
                    }

                    if (current_version < 106)
                    {
                        self->set_transaction_expiration(CDC_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC);
                    }
                    ...
                    {
                        self->set_last_scanned_block_number_for_cdc(0);
                    }
                    if (_unlocked_upgrade_tasks.empty())
                    {
                        self->set_version(CDC_WALLET_VERSION);
                ...
                const string& brainkey)
            {
                try {
                    ...
                    if (password.size() < CDC_WALLET_MIN_PASSWORD_LENGTH)
             ...
             *  returned will contain no more than CDC_BLOCKCHAIN_MAX_SLATE_SIZE delegates.
             */
            SlateEntry WalletImpl::get_delegate_slate(const VoteStrategy strategy)const
            {
                ...
                {
                    slate_size = std::min<size_t>(CDC_BLOCKCHAIN_MAX_SLATE_SIZE, for_candidates.size());
                }
                else if (strategy == vote_random)
                {
                    slate_size = std::min<size_t>(CDC_BLOCKCHAIN_MAX_SLATE_SIZE / 3, for_candidates.size());
                    slate_size = rand() % (slate_size + 1);
                }
                else if (strategy == vote_recommended && for_candidates.size() < CDC_BLOCKCHAIN_MAX_SLATE_SIZE)
                {
                    ...
                    while (for_candidates.size() < CDC_BLOCKCHAIN_MAX_SLATE_SIZE && recommended_candidate_ranks.size() > 0)
                    {
                        int best_rank = 0;
                ...
                FC_ASSERT(entry.slate.size() <= CDC_BLOCKCHAIN_MAX_SLATE_SIZE);
            ...
            const string& brainkey)
        {
            try {
                ...
                try
                {
                    ...
                    open(wallet_name);
                    unlock(password, CDC_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::create_from_json(const path& filename, const string& wallet_name, const string& passphrase)
        {
            try {
                ...
                try
                {
                    ...
                    close();
                    open(wallet_name);
                    unlock(passphrase, CDC_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::lock()
        {
            ...
            ilog("Wallet locked at time: ${t}", ("t", cdcchain::consensus::now()));
        ...
        bool Wallet::check_passphrase(const string& passphrase)
        {
            try {
                ...
                if (passphrase.size() < CDC_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
        ...
        void Wallet::change_passphrase(const string & old_passphrase, const string& new_passphrase)
        {
            try {
                if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
                if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
                if (new_passphrase.size() < CDC_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            const variant& private_data)
        {
            try {
                ...
                if (num_accounts_before == 0)
                {
                    ...
                    set_last_scanned_block_number_for_cdc(0);
                }
            ...
            const variant& private_data)
        {
            try {
                ...
                if (current_account.valid())
                {
                    ...
                    {
                        cdcchain::consensus::AccountEntry& as_blockchain_account_entry = *current_account;
                ...
                else
                {
                    ...
                    if (!current_registered_account.valid())
                    {
                        const time_point_sec now = cdcchain::consensus::now();
        ...
        void Wallet::accountsplit(const string & original, string & to_account, string & scdc_account)
        {
            if (original.size() < 66)
            {
                to_account = original;
            ...
            scdc_account = original.substr(original.size() - 32);
            if (INVALIDE_SCDC_ADDRESS == scdc_account)
            {
                scdc_account = "";
            }
            else
            {
                scdc_account = original;
            }
        }
            ...
            bool create_account)
        {
            try {
                ...
                if (blockchain_account_entry.valid())
                {
                    ...
                    account_entry->set_active_key(cdcchain::consensus::now(), active_public_key);
                    account_entry->last_update = cdcchain::consensus::now();
            ...
            bool create_account)
        {
            try {
                ...
                auto key = cdcchain::utilities::wif_to_key(wif_key);
                if (key.valid())
                {
        ...
        VoteSummary Wallet::get_vote_status(const string& account_name)
        {
            ...
            for (const auto& item : items)
            {
                ...
                for (const auto& entry : entrys)
                {
                    if (entry.asset_id() != 0)
                        continue;
                    if (entry.slate_id() != my_slate.id() && entry.balance > 1 * CDC_BLOCKCHAIN_PRECISION) // ignore dust
                    ...
                    total_possible += entry.get_spendable_balance(my->_blockchain->now()).amount * CDC_BLOCKCHAIN_MAX_SLATE_SIZE;
                }
            }
            ...
            bool sign)
        {
            try {
                ...
                const auto version = cdcchain::client::version_info()["client_version"].as_string();
        ...
        void Wallet::repair_entrys(const optional<string>& collecting_account_name)
        {
            try {
                ...
                for (const auto& item : items)
                {
                    ...
                    if (name.find(CDC_ADDRESS_PREFIX) != 0)
                        continue;
        ...
        std::map<std::string, cdcchain::consensus::DelegatePaySalary> Wallet::query_delegate_salarys()
        {
            FC_ASSERT(is_open());
            FC_ASSERT(is_unlocked());
            std::map<std::string, cdcchain::consensus::DelegatePaySalary> Salary_Map;
        ...
        WalletTransactionEntry Wallet::contract_register(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing)
        {
            ...
            FC_ASSERT(asset_rec->id == 0, "asset_symbol must be CDC");
        ...
        std::vector<cdcchain::consensus::Asset> Wallet::contract_register_testing(const string& owner, const fc::path codefile)
        {
            ...
            auto trans_entry = contract_register(owner,
                codefile,
                CDC_BLOCKCHAIN_SYMBOL,
                (double)asset_for_exec.amount / CDC_BLOCKCHAIN_PRECISION, 
                true
                );
            ...
            std::vector<cdcchain::consensus::Asset> asset_vec;
        ...
        std::vector<cdcchain::consensus::Asset> Wallet::contract_call_testing(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(caller), "Invalid account name");
        ...
        std::string Wallet::contract_call_offline(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            ...
            if (exception_code > 0)
            {
                cdcchain::consensus::contract_error con_err(32000, "exception", exception_msg);
                throw con_err;
            }
        ...
        cdcchain::wallet::oScriptEntry Wallet::get_script_entry(const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        cdcchain::consensus::ScriptIdType Wallet::script_add(const fc::path& filename, const string& description/*=string("")*/)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                FC_ASSERT(amount_to_transfer_symbol == CDC_BLOCKCHAIN_SYMBOL, "Asset symbol should be CDC");
                FC_ASSERT(data_ptr->is_valid_symbol(amount_to_transfer_symbol), "Invalid asset symbol");
                FC_ASSERT(my->is_receive_account(from_account_name), "Invalid account name");
		...
		std::vector<cdcchain::consensus::Asset> Wallet::contract_upgrade_testing(
			...
			)
		{
			try
			{
				...
				if (margin_balance_entry->balance != CDC_DEFAULT_CONTRACT_MARGIN)
					FC_CAPTURE_AND_THROW(invalid_margin_amount, (margin_balance_entry->balance));


				Asset margin = Asset(CDC_DEFAULT_CONTRACT_MARGIN);
				Asset fee = get_transaction_fee(0);
				Asset asset_for_exec = my->_blockchain->get_amount(CONTRACT_TESTING_LIMIT_MAX);
			...
			bool is_testing)
        {
            try
			{
				...
				FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not CDC!");
		...
		std::vector<cdcchain::consensus::Asset> Wallet::contract_destroy_testing(
			const Address& contract_id,
			...
			bool is_testing)
        {
            try
			{
                ...
                FC_ASSERT(balance_entry.valid(), "Can not get margin balance!");
                FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not CDC!");
                FC_ASSERT(balance_entry->balance == CDC_DEFAULT_CONTRACT_MARGIN, "invalid margin balance amount");
                ...
                {
                    FC_ASSERT(balance_entry->asset_id() == 0, "contract balance asset is not CDC!");
                }
            ...
            bool sign)
        {
            try {
                ...
                {
                    required_fees += Asset(CDC_BLOCKCHAIN_REGISTER_ACCOUNT_FEE, 0);
                }
        ...
        {
            try {
                FC_ASSERT(cdcchain::consensus::is_power_of_ten(precision));
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        void Wallet::set_transaction_fee(const Asset& fee)
        {
            try {
                ...
                if (fee.amount < CDC_DEFAULT_TRANSACTION_FEE || fee.asset_id != 0)
        ...
        Asset Wallet::get_transaction_imessage_fee(const std::string & imessage)const
        {
            try
            {
                ...
                Asset require_fee;
                    ...
                    if (CDC_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE >= imessage.size())
                    {
                        return require_fee;
                    }
                    auto min_fee_coe = get_transaction_imessage_fee_coe();
                    require_fee.amount = min_fee_coe * (imessage.size() - CDC_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
        ...
        Asset Wallet::get_transaction_fee(const AssetIdType desired_fee_asset_id)const
        {
            try {
                ...
                Asset xts_fee(CDC_WALLET_DEFAULT_TRANSACTION_FEE, 0);
                try
                {
        ...
        void Wallet::set_last_scanned_block_number_for_cdc(uint32_t block_num)
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
        ...
        uint32_t Wallet::get_last_scanned_block_number_for_cdc()const
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                try
        ...
        void Wallet::set_transaction_imessage_fee_coe(const ImessageIdType& coe)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (coe < CDC_BLOCKCHAIN_MIN_MESSAGE_FEE_COE)
                    {
        ...
        void Wallet::set_transaction_imessage_soft_max_length(const ImessageLengthIdType& length)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (length < 0 || length > CDC_BLOCKCHAIN_MAX_MESSAGE_SIZE)
                    {
        ...
        void Wallet::set_transaction_expiration(uint32_t secs)
        {
            try {
                ...
                if (secs > CDC_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC)
