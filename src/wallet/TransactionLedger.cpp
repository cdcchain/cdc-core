...
using namespace cdcchain::wallet::detail;
    ...
    bool bNeedcreate)
{
    ...
    // 
        ...
        {
            if (store_entry && transaction.cdc_account != "")
            {
                CdcTrxidBalance cdc_entry;
                ...
                for (const auto item : pretty_trx.ledger_entries)
                {
                    ...
                    {
                        cdc_entry.from_account = item.from_account;
                        break;
                    }
                }
                string strToAccount;
                string strSubAccount;
                self->accountsplit(transaction.cdc_account, strToAccount, strSubAccount);
                auto blockchain_trx = _blockchain->get_transaction(transaction_id);
                bool bDiffDeop = false;
                if (blockchain_trx.valid())
                {
                    ShareType cdc_amount = 0;
                    string preowner;
                    int iLoop = 0;
                ...
                //cdc_entry.from_account = transaction.from_account;
                cdc_entry.create_time = transaction_entry->created_time;
                cdc_entry.cdc_account = transaction.cdc_account;
                cdc_entry.block_num = transaction_entry->block_num;
                transaction_entry->trx.cdc_account = transaction.cdc_account;
                transaction_entry->trx.cdc_inport_asset = transaction.cdc_inport_asset;
                cdc_entry.trx_id = transaction.id();
                    //cdc_entry.asset_trx = transaction.cdc_inport_asset;
                    cdc_entry.asset_trx = Asset(cdc_amount, 0);
                    {
                        writelock wlock(m_mutex_for_cdc);
                _blockchain->transaction_insert_to_cdc_full_entry(strToAccount, cdc_entry);
                //_blockchain->transaction_insert_to_cdc_balance(cdc_entry.cdc_account, cdc_entry);
                    }
                }
            }
        }
...
bool cdcchain::wallet::detail::WalletImpl::scan_withdraw_contract(const WithdrawContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
...
bool cdcchain::wallet::detail::WalletImpl::scan_ondestroycontract_withdraw(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
		const auto amount = Asset(op.amount); // Always base asset
...
bool cdcchain::wallet::detail::WalletImpl::scan_ondestroycontract_deposit(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try {
...
bool cdcchain::wallet::detail::WalletImpl::scan_deposit_contract(const DepositContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try{
		auto entry = LedgerEntry();
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        if (!asset_symbol.empty() && asset_symbol != CDC_BLOCKCHAIN_SYMBOL)
        {
            try
            {
        ...
        for (const auto& item : transactions)
        {
            ...
            if (!tx_entry.is_confirmed && (tx_entry.trx.expiration < cdcchain::consensus::now()))
            {
                continue;
            }
...
PrettyTransaction		Wallet::to_pretty_trx(const cdcchain::consensus::TransactionEntry& trx_entry, const std::string addr_for_fee) const
{
    ...
    pretty_trx.trx_type = cdcchain::consensus::TransactionType::normal_transaction;
    ...
    for (const auto& op : trx.operations)
    {
        ...
        case register_account_op_type:
        {
            ...
            if (register_op.delegate_pay_rate > 100)
            {
                ...
                pretty_trx.trx_type = cdcchain::consensus::TransactionType::register_account_transaction;
            }
            else
            {
                pretty_entry.memo = "register delegate " + account_name_entry->name; // Can't tell if initially registered as a delegate
                pretty_trx.trx_type = cdcchain::consensus::TransactionType::register_delegate_transaction;
            }
		...
		case update_account_op_type:
		{
            ...
            if (total_fee <= Asset( 1000 * CDC_BLOCKCHAIN_PRECISION ))
            {
                ...
                pretty_trx.trx_type = cdcchain::consensus::TransactionType::update_account_transaction;
            }
            else
            {
                pretty_entry.memo = "upgrade " + account_name_entry->name;
                pretty_trx.trx_type = cdcchain::consensus::TransactionType::upgrade_account_transaction;
            }
...
PrettyContractTransaction		Wallet::to_pretty_contract_trx(const cdcchain::consensus::TransactionEntry& trx_entry) const
{
    PrettyContractTransaction pretty_trx;
    SignedTransaction result_trx = trx_entry.trx;

    FC_ASSERT((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type == cdcchain::consensus::OperationTypeEnum::transaction_op_type) , 
        "not result contract transaction");
    SignedTransaction orig_trx = trx_entry.trx.operations[0].as<cdcchain::consensus::TransactionOperation>().trx;
    ...
    FC_ASSERT((orig_trx.operations.size() > 0) && 
        ((orig_trx.operations[0].type >= fc::enum_type<uint8_t, cdcchain::consensus::OperationTypeEnum>(contract_register_op_type))
            && (orig_trx.operations[0].type <= fc::enum_type<uint8_t, cdcchain::consensus::OperationTypeEnum>(transfer_contract_op_type))), "invalid original contract transaction");


    cdcchain::consensus::OperationTypeEnum contract_op_type = orig_trx.operations[0].type;
    ...
    if (contract_op_type == cdcchain::consensus::OperationTypeEnum::contract_register_op_type)
    {
        pretty_trx.trx_type = cdcchain::consensus::TransactionType::contract_register_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        cdcchain::consensus::RegisterContractOperation contract_register_op = orig_trx.operations[0].as<cdcchain::consensus::RegisterContractOperation>();
        Address contract_owner = cdcchain::consensus::Address(contract_register_op.owner);
        ...
        to_contract_ledger_entry.amount = Asset(CDC_DEFAULT_CONTRACT_MARGIN, 0);
        ...
        for (const auto& op : result_trx.operations)
        {
            ...
            if (op.type == fc::enum_type<uint8_t, cdcchain::consensus::OperationTypeEnum>(balances_withdraw_op_type))
            {
    ...
    {
        pretty_trx.trx_type = cdcchain::consensus::TransactionType::contract_upgrade_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        cdcchain::consensus::UpgradeContractOperation contract_upgrade_op = orig_trx.operations[0].as<cdcchain::consensus::UpgradeContractOperation>();
    ...
    else if (contract_op_type == cdcchain::consensus::OperationTypeEnum::contract_destroy_op_type)
    {
        pretty_trx.trx_type = cdcchain::consensus::TransactionType::contract_destroy_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        cdcchain::consensus::DestroyContractOperation contract_destroy_op = orig_trx.operations[0].as<cdcchain::consensus::DestroyContractOperation>();
    ...
    else if (contract_op_type == cdcchain::consensus::OperationTypeEnum::contract_call_op_type)
    {
        pretty_trx.trx_type = cdcchain::consensus::TransactionType::contract_call_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        cdcchain::consensus::CallContractOperation contract_call_op = orig_trx.operations[0].as<cdcchain::consensus::CallContractOperation>();
    ...
    else if (contract_op_type == cdcchain::consensus::OperationTypeEnum::transfer_contract_op_type)
    {
        pretty_trx.trx_type = cdcchain::consensus::TransactionType::transfer_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        cdcchain::consensus::TransferContractOperation transfer_contract_op = orig_trx.operations[0].as<cdcchain::consensus::TransferContractOperation>();
        Address contract_depositer = cdcchain::consensus::Address(transfer_contract_op.from);
