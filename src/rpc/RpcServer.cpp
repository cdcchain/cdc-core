...
namespace cdcchain {
    namespace rpc {
        ...
        namespace detail
        {
            class RpcServerImpl : public cdcchain::rpc_stubs::CommonApiRpcServer
            {
            public:
                ...
                RpcServerImpl(cdcchain::client::Client* client) :
                    _client(client),
                ...
                virtual cdcchain::api::CommonApi* get_client() const override;
                ...
                virtual void store_method_metadata(const cdcchain::api::MethodData& method_metadata);

                std::string help(const std::string& command_name) const;

                std::string make_short_description(const cdcchain::api::MethodData& method_data, bool show_decription = true) const
                {
                    std::string help_string;
                    std::stringstream short_description;
                    short_description << std::setw(100) << std::left;
                    help_string = method_data.name + " ";
                    for (const cdcchain::api::ParameterData& parameter : method_data.parameters)
                    {
                        if (parameter.classification == cdcchain::api::required_positional)
                            help_string += std::string("<") + parameter.name + std::string("> ");
                        else if (parameter.classification == cdcchain::api::optional_named)
                            help_string += std::string("{") + parameter.name + std::string("} ");
                        else if (parameter.classification == cdcchain::api::required_positional_hidden)
                            continue;
                        else
                            help_string += std::string("[") + parameter.name + std::string("] ");
                    }
                ...
                fc::http::reply::status_code handle_http_rpc(const fc::http::request& r, const fc::http::server::response& s)
                {
                    ...
                    try {
                        ...
                        if (fc::time_point(cdcchain::consensus::now()) - _last_cache_clear_time > fc::seconds(CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC))
                        {
                            _last_cache_clear_time = cdcchain::consensus::now();
                            _call_cache.clear();
                        }
                ...
                void encrypted_accept_loop(const cdcchain::consensus::PrivateKeyType& server_key)
                {
                    while (!_encrypted_accept_loop_complete.canceled())
                    {
                        cdcchain::network::StcpSocketPtr sock = std::make_shared<cdcchain::network::StcpSocket>();
                        try
                        {
                    ...
                    const fc::variants& arguments)
                {
                    ...
                    if ((method_data.prerequisites & cdcchain::api::json_authenticated) &&
                    ...
                    const fc::variants& arguments_from_caller)
                {
                    ...
                    if (method_data.prerequisites & cdcchain::api::wallet_open)
                        verify_wallet_is_open();
                    if (method_data.prerequisites & cdcchain::api::wallet_unlocked)
                        verify_wallet_is_unlocked();
                    if (method_data.prerequisites & cdcchain::api::connected_to_network)
                        verify_connected_to_network();
                    if (method_data.prerequisites & cdcchain::api::simulator_open)
                    ...
                    {
                        if (parameter.classification == cdcchain::api::required_positional
                            || parameter.classification == cdcchain::api::required_positional_hidden)
                        {
                        ...
                        else if (parameter.classification == cdcchain::api::optional_positional)
                        {
            ...
            cdcchain::api::CommonApi* RpcServerImpl::get_client() const
            {
                return _client;
            }
            ...
            void RpcServerImpl::store_method_metadata(const cdcchain::api::MethodData& method_metadata)
            {
            ...
            std::string RpcServerImpl::help(const std::string& command_name) const
            {
                ...
                { //display detailed description of requested command
                    ...
                    if (alias_iter != _alias_map.end())
                    {
                        ...
                        const cdcchain::api::MethodData& method_data = method_iter->second;
                        help_string += "Usage:\n";
        ...
        RpcServer::RpcServer(cdcchain::client::Client* client) :
        ...
        const cdcchain::api::MethodData& RpcServer::get_method_data(const std::string& method_name)
        {
        ...
        std::vector<cdcchain::api::MethodData> RpcServer::get_all_method_data() const
        {
            std::vector<cdcchain::api::MethodData> result;
            for (const detail::RpcServerImpl::MethodMapType::value_type& value : my->_method_map)
                result.emplace_back(value.second);
            return result;
        }

        void RpcServer::register_method(cdcchain::api::MethodData data)
        {
