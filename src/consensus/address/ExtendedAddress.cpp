    ...
    namespace consensus {
        ...
        ExtendedAddress::ExtendedAddress(const std::string& base58str)
        {
            try {
                ...
                FC_ASSERT(base58str.size() > prefix_len, "Address must be longer than CDC_ADDRESS_PREFIX");
                FC_ASSERT(base58str.substr(0, prefix_len) == prefix, "CDC_ADDRESS_PREFIX must be in front of the address");
...
namespace fc
{
    void to_variant(const cdcchain::consensus::ExtendedAddress& ext_addr, fc::variant& vo)
    {
    ...
    {
        vo = cdcchain::consensus::ExtendedAddress(var.as_string());
    }
}
