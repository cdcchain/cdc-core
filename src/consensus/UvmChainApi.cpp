        ...
        namespace api {
            ...
            */
                ...
                const char *asset_type, int64_t amount)
            {
                ...
                try
                {
                    if (!Address::is_valid(contract_address, AddressType::contract_address))
                        return -3;
                    if (!Address::is_valid(to_addr, AddressType::cdc_address))
                        return -4;
			...
			bool UvmChainApi::is_valid_address(lua_State *L, const char *address_str)
			{
                try
                {
                    ...
                    return Address::is_valid(addr, AddressType::cdc_address);
                }
                catch (const fc::exception&)
                {
                    return false;
			...
			bool UvmChainApi::is_valid_contract_address(lua_State *L, const char *address_str)
			{
                ...
                {
                    std::string addr(address_str);
					return Address::is_valid(addr, AddressType::contract_address);
                }
                catch (const fc::exception&)
                {
                    return false;
			...
			static int32_t appoint_contract_role(lua_State *L, const char* user_address, const char* contract_id, const RoleTypeEnum& role_type, const RoleSubTypeEnum& role_sub_type)
			{
				...
				try {
					...
					{
                        ContractOperatorRole contract_operator_role;
			...
			static int32_t revoke_contract_role(lua_State *L, const char* user_address, const char* contract_id, const RoleTypeEnum& role_type)
			{
				...
				try {
					...
					for (auto iter = entry.role_cond_vec.begin(); iter != entry.role_cond_vec.end(); ) {
                        ...
                        if (iter->role_address == Address(std::string(user_address), AddressType::cdc_address) && iter->role_type == role_type) {
                            iter = entry.role_cond_vec.erase(iter);
                        }
                        else
                            iter++;
