...
namespace cdcchain {
    namespace consensus {
            ...
            uint64_t precision)
        {
            ...
            FC_ASSERT(max_share_supply <= CDC_BLOCKCHAIN_MAX_SHARES, "Max_share_supply must not be bigger than CDC_BLOCKCHAIN_MAX_SHARES!");
        ...
        cdcchain::consensus::ContractIdType Transaction::contract_register(const uvm::blockchain::Code& code, const PublicKeyType& owner, const Asset& initlimit,const Asset& transaction_fee, const map<BalanceIdType, ShareType>& balances)
        {
