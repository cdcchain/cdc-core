    ...
    namespace consensus {
        ...
        void TransactionEvaluationState::post_evaluate()
        {
            try {
                ...
                /* if( _current_state->get_head_block_num() >= CDC_V0_6_4_FORK_BLOCK_NUM )
                {
                for( const auto& op : trx.operations )
                {
                if( operation_type_enum( op.type ) == cover_op_type )
                {
                FC_ASSERT( balance[ 0 ] <= CDC_BLOCKCHAIN_PRECISION );
                }
                }
                }*/
            } FC_CAPTURE_AND_RETHROW()
        ...
        void TransactionEvaluationState::evaluate(const SignedTransaction& trx_arg)
        {
            ...
            try {
                ...
                try {
                    ...
                    if (origin_trx_basic_verify(trx) && (trx.data_size() > CDC_BLOCKCHAIN_MAX_TRX_SIZE))
                    {
					...
					if (num_of_signature > CDC_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)
					{
						FC_CAPTURE_AND_THROW(too_much_signature, (CDC_BLOCKCHAIN_MAX_SIGNAGTURE_NUM)(num_of_signature));
					}
        ...
        SignedTransaction TransactionEvaluationState::simulator_evaluate(const SignedTransaction &trx_arg, bool& ignore_check_required_fee)
        {
            try
            {
                ...
                try
                {
                    ...
                    if ((_current_state->now() + CDC_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration)
                        FC_CAPTURE_AND_THROW(invalid_transaction_expiration, (trx_arg)(_current_state->now()));
        ...
        bool TransactionEvaluationState::is_contract_op(const cdcchain::consensus::OperationTypeEnum& op_type)const
        {
            if (op_type >= cdcchain::consensus::OperationTypeEnum::contract_register_op_type
                && op_type <= cdcchain::consensus::OperationTypeEnum::transfer_contract_op_type)
                return true;
            return false;
        }
        ...
        bool TransactionEvaluationState::origin_trx_basic_verify(const SignedTransaction& trx)const
        {
            ...
            for (const auto& op : trx.operations)
            {
                if (op.type == cdcchain::consensus::OperationTypeEnum::transaction_op_type)
                    return false;
