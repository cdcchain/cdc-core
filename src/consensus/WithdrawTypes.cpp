...
namespace cdcchain {
    namespace consensus {
            ...
            bool use_stealth_address)
        {
            ...
            if (memo_message.size() <= CDC_BLOCKCHAIN_MAX_MEMO_SIZE)
            {
    ...
    void from_variant(const variant& var, cdcchain::consensus::MemoData& vo)
    {
        try {
            ...
            if (obj.contains("from"))
                vo.from = obj["from"].as<cdcchain::consensus::PublicKeyType>();
            ...
            if (obj.contains("memo_flags"))
                vo.memo_flags = obj["memo_flags"].as<cdcchain::consensus::MemoFlagsEnum>();
