...
namespace cdcchain {
    namespace consensus {
        ...
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
            if (!prev_state) return get_slot_start_time(cdcchain::consensus::now());
...
// 			}
        ...
        cdcchain::consensus::BlockIdType PendingChainState::get_block_id(uint32_t block_num) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
        ...
        cdcchain::consensus::SignedBlockHeader PendingChainState::get_block_header(const BlockIdType& id) const
        {
            const ChainInterfacePtr prev_state = _prev_state.lock();
		...
		cdcchain::consensus::oContractTemplateEntry PendingChainState::contracttemplate_lookup_by_hash(const std::string& hash)const
		{
