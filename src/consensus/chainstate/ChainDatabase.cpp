...
namespace cdcchain {
    namespace consensus {
        ...
        namespace detail
        {
            void ChainDatabaseImpl::revalidate_pending()
            {
                ...
                while (itr.valid())
                {
                    ...
                    try
                    {
                        ...
                        count++;
                        if (count > CDC_BLOCKCHAIN_REVALIDATE_MAX_TRX_COUNT)
                            break;
            ...
            bool ChainDatabaseImpl::replay_required(const fc::path& data_dir)
            {
                try {
                    ...
                    return !entry.valid() || entry->value.as_uint64() != CDC_BLOCKCHAIN_DATABASE_VERSION;
                } FC_CAPTURE_AND_RETHROW((data_dir))
            }
            ...
            void ChainDatabaseImpl::open_database(const fc::path& data_dir)
            {
                try {
                    ...
                    _cdc_input_balance_entry.open(data_dir / "index/_cdc_input_balance_entry");
                    _cdc_full_entry.open(data_dir / "index/_cdc_full_entry");
            ...
            void ChainDatabaseImpl::clear_invalidation_of_future_blocks()
            {
                for (auto block_id_itr = _revalidatable_future_blocks_db.begin(); block_id_itr.valid();)
                {
                    ...
                    fc::time_point_sec now = cdcchain::consensus::now();
            ...
            DigestType ChainDatabaseImpl::initialize_genesis(const optional<path>& genesis_file, const bool statistics_enabled)
            {
                try {
                    ...
                    // Check genesis state
                    FC_ASSERT(config.delegates.size() >= CDC_BLOCKCHAIN_NUM_DELEGATES,
                        "genesis.json does not contain enough initial delegates!",
                        ("required", CDC_BLOCKCHAIN_NUM_DELEGATES)("provided", config.delegates.size()));
                    ...
                    {
                        //static const vector<string> cdc_prefixes{ "CDC" };
                        try
                        {
                        ...
                        {
                            //for( const string& prefix : cdc_prefixes )
                            //{
                            // if( raw_address.find( prefix ) == 0 )
                            //  {
                            //  return address( CDC_ADDRESS_PREFIX + raw_address.substr( prefix.size() ) );
                            // }
                            //}
                        }
                    ...
                    base_asset.symbol = CDC_BLOCKCHAIN_SYMBOL;
                    base_asset.name = CDC_BLOCKCHAIN_NAME;
                    base_asset.description = CDC_BLOCKCHAIN_DESCRIPTION;
                    base_asset.public_data = variant("");
                    base_asset.issuer_account_id = 0;
                    base_asset.precision = CDC_BLOCKCHAIN_PRECISION;
                    base_asset.registration_date = timestamp;
                    base_asset.last_update = timestamp;
                    base_asset.current_share_supply = total_base_supply;
                    base_asset.maximum_share_supply = CDC_BLOCKCHAIN_MAX_SHARES;
                    ...
                    for (const GenesisAsset& asset : config.market_assets)
                    {
                        ...
                        rec.maximum_share_supply = CDC_BLOCKCHAIN_MAX_SHARES;
                ...
                const FullBlock& block_data)
            {
                try {
                    ...
                    if (self->get_statistics_enabled())
                    {
                        ...
                        entry.latency = cdcchain::consensus::now() - block_data.timestamp;
            ...
            {
                try {
                    // if( pending_state->get_head_block_num() < CDC_V0_6_0_FORK_BLOCK_NUM )
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    if (block_num > CDC_BLOCKCHAIN_MAX_UNDO_HISTORY)
                    {
                        const uint32_t old_block_num = block_num - CDC_BLOCKCHAIN_MAX_UNDO_HISTORY;
            ...
            void ChainDatabaseImpl::verify_header(const DigestBlock& block_digest, const PublicKeyType& block_signee)const
            {
                try {
                    ...
                    if (block_digest.timestamp.sec_since_epoch() % CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0)
                    ...
                    if (block_digest.timestamp > (now + CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
            ...
            /**
             *  A block should be produced every CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
                ...
                const PendingChainStatePtr& pending_state)const
            {
                try {
                    ...
                    if (head_block.block_num > 0) block_timestamp = head_block.timestamp + CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    ...
                    for (; block_timestamp < block_header.timestamp;
                        block_timestamp += CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
                    ...
                    if (required_confirmations > CDC_BLOCKCHAIN_NUM_DELEGATES * 3)
                        required_confirmations = 3 * CDC_BLOCKCHAIN_NUM_DELEGATES;
            ...
            {
                try {//todo remove????
                    // if( pending_state->get_head_block_num() < CDC_V0_7_0_FORK_BLOCK_NUM )
                    // return update_active_delegate_list_v1( block_num, pending_state );

                    if (block_num % CDC_BLOCKCHAIN_NUM_DELEGATES != 0)
                        return;
            ...
            void ChainDatabaseImpl::extend_chain(const FullBlock& block_data)
            {
                try {
                    ...
                    try
                    {
                        ...
                        if (cdcchain::client::g_client->get_wallet() != nullptr&&cdcchain::client::g_client->get_wallet()->is_open())
                        {
                            if (!cdcchain::client::g_client->get_wallet()->get_my_delegates(cdcchain::wallet::enabled_delegate_status).empty())
                            {
                                auto fee = cdcchain::client::g_client->get_delegate_config().transaction_min_fee;
                                _relay_fee = fee > CDC_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee : CDC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                            else
                            {
                                auto fee = cdcchain::client::g_client->get_wallet()->get_transaction_fee();
                                _relay_fee = fee.amount > CDC_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee.amount : CDC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                                //_relay_fee = CDC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
                            }
                        }
                    }
                    catch (const fc::exception& e)
                    ...
                    if ((now() - block_data.timestamp).to_seconds() < CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
            ...
            void ChainDatabaseImpl::pop_block()
            {
                try {
                    ...
                    cdcchain::consensus::PendingChainStatePtr undo_state_ptr = std::make_shared<cdcchain::consensus::PendingChainState>(undo_state);
        ...
        {
            return get_delegates_by_vote(0, CDC_BLOCKCHAIN_NUM_DELEGATES);
        }
            ...
            try {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        my->open_database(data_dir);
                        store_property_entry(PropertyIdType::database_version, variant(CDC_BLOCKCHAIN_DATABASE_VERSION));
                        ...
                        const auto start_time = cdcchain::consensus::now();
                        ...
                        const auto insert_block = [&](const FullBlock& block)
                        {
                            if (blocks_indexed % 200 == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    progress = float(blocks_indexed * CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC) / seconds;
                                }
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (last_known_block_num > CDC_BLOCKCHAIN_MAX_UNDO_HISTORY)
                                    my->_min_undo_block = last_known_block_num - CDC_BLOCKCHAIN_MAX_UNDO_HISTORY;
                        ...
                        catch (cdcchain::consensus::store_and_index_a_seen_block&  e)
                        {
                        ...
                        std::cout << "\rSuccessfully replayed " << blocks_indexed << " blocks in "
                            << (cdcchain::consensus::now() - start_time).to_seconds() << " seconds.                          "
        ...
        void ChainDatabase::close()
        {
            try {
                ...
                my->_cdc_input_balance_entry.close();
                my->_cdc_full_entry.close();
        ...
        {
            try {
                auto slot_number = cdcchain::consensus::get_slot_number(timestamp);
                auto delegate_pos = slot_number % CDC_BLOCKCHAIN_NUM_DELEGATES;
        ...
        {
            try {
                auto next_block_time = cdcchain::consensus::get_slot_start_time(cdcchain::consensus::now());
                if (next_block_time <= now()) next_block_time += CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                auto last_block_time = next_block_time + (CDC_BLOCKCHAIN_NUM_DELEGATES * CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

                auto active_delegates = get_active_delegates();
                for (; next_block_time < last_block_time; next_block_time += CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
                {
                    auto slot_number = cdcchain::consensus::get_slot_number(next_block_time);
                    auto delegate_pos = slot_number % CDC_BLOCKCHAIN_NUM_DELEGATES;
            ...
            const ShareType required_fees, bool contract_vm_exec, bool skip_signature_check, bool throw_exec_exception)
        {
            try {
                ...
                catch (cdcchain::consensus::ignore_check_required_fee_state& e)
                {
        ...
        BlockForkData ChainDatabase::push_block(const FullBlock& block_data)
        {
            try {
                ...
                if (head_block_num > CDC_BLOCKCHAIN_MAX_UNDO_HISTORY &&
                    block_data.block_num <= (head_block_num - CDC_BLOCKCHAIN_MAX_UNDO_HISTORY))
                {
                    elog("block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
                        ...
                        ("head_block_num", head_block_num)("undo_history", CDC_BLOCKCHAIN_MAX_UNDO_HISTORY));
                ...
                */
                ...
                else
                {
                    ...
                    cdcchain::consensus::ntp_time();
                    // 		 cdcchain::consensusupdate_ntp_time();
        ...
        fc::time_point_sec ChainDatabase::now()const
        {
            ...
            {
                auto slot_number = cdcchain::consensus::get_slot_number(cdcchain::consensus::now());
                return cdcchain::consensus::get_slot_start_time(slot_number - 1);
            }
        ...
        */
        double ChainDatabase::get_average_delegate_participation()const
        {
            try {
                ...
                const auto now = cdcchain::consensus::now();
                if (head_num < 1)
                {
                    return 0;
                }
                else if (head_num <= CDC_BLOCKCHAIN_NUM_DELEGATES)
                {
                    // what percent of the maximum total blocks that could have been produced
                    // have been produced.
                    const auto expected_blocks = (now - get_block_header(1).timestamp).to_seconds() / CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                ...
                else
                {
                    ...
                    const auto starting_time = get_block_header(head_num - CDC_BLOCKCHAIN_NUM_DELEGATES).timestamp;
                    const auto expected_production = (now - starting_time).to_seconds() / CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    return 100 * double(CDC_BLOCKCHAIN_NUM_DELEGATES) / expected_production;
                }
            } FC_RETHROW_EXCEPTIONS(warn, "")
        }
        ...
        TransactionEvaluationStatePtr ChainDatabase::store_pending_transaction(const SignedTransaction& trx, bool override_limits, bool contract_vm_exec, bool cache)
        {
            try {
                ...
                if (!override_limits)
                {
                    if (my->_pending_fee_index.size() > CDC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE)
                    {
                        auto overage = my->_pending_fee_index.size() - CDC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
        ...
        FullBlock ChainDatabase::generate_block(const time_point_sec block_timestamp, const DelegateConfig& config)
        {
            try {
                ...
                //if( pending_state->get_head_block_num() >= CDC_V0_4_9_FORK_BLOCK_NUM )
                        ...
                        try
                        {
                            ...
                            auto pending_trx_state = origin_trx_state;
                            {
                                ...
                                if (trx_eval_state->p_result_trx.operations.size() > 0)
                                {
                                    ...
                                    if (iMessageLength > CDC_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
                                    {
                                        imessage_fee = config.transaction_imessage_min_fee_coe * (iMessageLength - CDC_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
                                    }
        ...
        string ChainDatabase::export_fork_graph(uint32_t start_block, uint32_t end_block, const fc::path& filename)const
        {
            ...
            for (auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr)
            {
                ...
                {
                    unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
        ...
        bool ChainDatabase::store_account_entries_for_simulator(const vector<cdcchain::wallet::WalletAccountEntry>& vec_account_entry)
        {
        ...
        vector<CdcTrxidBalance> ChainDatabase::fetch_cdc_input_balance(const uint32_t & block_num)
        {
            try{
                vector < CdcTrxidBalance > results;
                for (auto iter = my->_cdc_input_balance_entry.unordered_begin();
                    iter != my->_cdc_input_balance_entry.unordered_end(); ++iter)
                {
                    CdcTrxidBalance ubTemp;
                    ubTemp.block_num = block_num + 1;
                    set<CdcTrxidBalance>::iterator block_iter = iter->second.lower_bound(ubTemp);
        ...
        vector<CdcTrxidBalance> ChainDatabase::fetch_cdc_full_entry(const uint32_t& block_num, const uint32_t & last_scan_block_num)
        {
            try{
                vector < CdcTrxidBalance > results;
                ...
                for (auto iter = my->_cdc_full_entry.unordered_begin();
                    iter != my->_cdc_full_entry.unordered_end(); ++iter)
                {
                    auto block_iter = iter->second.cdc_block_sort.lower_bound(block_num + 1);
                    for (; block_iter != iter->second.cdc_block_sort.end(); block_iter++)
                    {
        ...
        void ChainDatabase::transaction_insert_to_cdc_full_entry(const string & cdc_account, const CdcTrxidBalance & cdc_balance_record)
        {
            auto iter_entry = my->_cdc_full_entry.unordered_find(cdc_account);
            CdcBalanceEntry ubentrys;
            if (iter_entry != my->_cdc_full_entry.unordered_end())
            {
                ubentrys = iter_entry->second;
                if (ubentrys.cdc_trxid_sort.count(cdc_balance_record.trx_id) != 0)
                {
                    return;
                }
            }
            ubentrys.cdc_trxid_sort.insert(cdc_balance_record.trx_id);
            ubentrys.cdc_block_sort.insert(std::make_pair(cdc_balance_record.block_num, cdc_balance_record));
            my->_cdc_full_entry.store(cdc_account, ubentrys);
        }
        void ChainDatabase::transaction_erase_from_cdc_full_entry(const string & cdc_account, const CdcTrxidBalance & cdc_balance_record)
        {
            auto iter_entry = my->_cdc_full_entry.unordered_find(cdc_account);
            if (iter_entry != my->_cdc_full_entry.unordered_end())
            {
                CdcBalanceEntry ubentrys = iter_entry->second;
                ubentrys.cdc_trxid_sort.erase(cdc_balance_record.trx_id);
                auto iter = ubentrys.cdc_block_sort.lower_bound(cdc_balance_record.block_num);
                for (; iter != ubentrys.cdc_block_sort.end();)
                {
                    if (iter->second.block_num != cdc_balance_record.block_num)
                    {
                        break;
                    }
                    if (iter->second.trx_id == cdc_balance_record.trx_id)
                    {
                        iter = ubentrys.cdc_block_sort.erase(iter);
                    }
                    else
                    {
                        ++iter;
                ...
                if (!(ubentrys.cdc_block_sort.empty()))
                {
                    my->_cdc_full_entry.store(cdc_account, ubentrys);
                }
                else
                {
                    my->_cdc_full_entry.remove(cdc_account);
                }
            }
        }
        void ChainDatabase::transaction_insert_to_cdc_balance(const string & cdc_account, const CdcTrxidBalance & cdc_balance_entry)
        {
            auto iter_ids = my->_cdc_input_balance_entry.unordered_find(cdc_account);
            set<CdcTrxidBalance> ids;
            if (iter_ids != my->_cdc_input_balance_entry.unordered_end())
            {
                ids = iter_ids->second;
            }
            ids.insert(cdc_balance_entry);
            my->_cdc_input_balance_entry.store(cdc_account, ids);
        }
        void ChainDatabase::transaction_erase_from_cdc_balance(const string & cdc_account, const CdcTrxidBalance & cdc_balance_entry)
        {
            auto iter_ids = my->_cdc_input_balance_entry.unordered_find(cdc_account);
            if (iter_ids != my->_cdc_input_balance_entry.unordered_end())
            {
                auto ids = iter_ids->second;
                ids.erase(cdc_balance_entry);
                if (!(ids.empty()))
                {
                    my->_cdc_input_balance_entry.store(cdc_account, ids);
                }
                ...
                {
                    my->_cdc_input_balance_entry.remove(cdc_account);
                }
            }
        }
        ...
        void ChainDatabase::dump_state(const fc::path& path)const
        {
            try {
                ...
                */
                ...
                ulog("Dumped ${p}", ("p", next_path));
                next_path = dir / "cdc_transaction_balance_db.json";
                my->_cdc_input_balance_entry.export_to_json(next_path);
				ulog("Dumped ${p}", ("p", next_path));
        ...
        oResultTIdEntry cdcchain::consensus::ChainDatabase::contract_lookup_resultid_by_reqestid(const TransactionIdType & id) const
        {
        ...
        void cdcchain::consensus::ChainDatabase::contract_store_resultid_by_reqestid(const TransactionIdType & req, const ResultTIdEntry & res)
        {
            my->_request_to_result_iddb.store(req, res);
        }

        void cdcchain::consensus::ChainDatabase::contract_erase_resultid_by_reqestid(const TransactionIdType & req)
        {
            my->_request_to_result_iddb.remove(req);
        }

        oRequestIdEntry cdcchain::consensus::ChainDatabase::contract_lookup_requestid_by_resultid(const TransactionIdType &id) const
        {
        ...
        void cdcchain::consensus::ChainDatabase::contract_store_requestid_by_resultid(const TransactionIdType & res, const RequestIdEntry & req)
        {
            my->_result_to_request_iddb.store(res, req);
        }

        void cdcchain::consensus::ChainDatabase::contract_erase_requestid_by_resultid(const TransactionIdType & result)
        {
            my->_result_to_request_iddb.remove(result);
        }

        oContractinTrxEntry cdcchain::consensus::ChainDatabase::contract_lookup_contractid_by_trxid(const TransactionIdType &id) const
        {
        ...
        oContractTrxEntry cdcchain::consensus::ChainDatabase::contract_lookup_trxid_by_contract_id(const ContractIdType &id) const
        {
        ...
        void cdcchain::consensus::ChainDatabase::contract_store_contractid_by_trxid(const TransactionIdType & tid, const ContractinTrxEntry & cid)
        {
            my->_trx_to_contract_iddb.store(tid, cid);
        }

        void cdcchain::consensus::ChainDatabase::contract_store_trxid_by_contractid(const ContractIdType & cid, const ContractTrxEntry & tid)
        {
            my->_contract_to_trx_iddb.store(cid, tid);
        }

        void cdcchain::consensus::ChainDatabase::contract_erase_trxid_by_contract_id(const ContractIdType &id)
        {
            my->_contract_to_trx_iddb.remove(id);
        }

        void cdcchain::consensus::ChainDatabase::contract_erase_contractid_by_trxid(const TransactionIdType & tid)
        {
