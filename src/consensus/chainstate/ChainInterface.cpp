...
namespace cdcchain {
    namespace consensus {
        ...
        {
            try {
                if (name.size() < CDC_BLOCKCHAIN_MIN_NAME_SIZE) return false;
                if (name.size() > CDC_BLOCKCHAIN_MAX_NAME_SIZE) return false;
        ...
        {
            try {
                // if( get_head_block_num() < CDC_V0_5_0_FORK_BLOCK_NUM )
                // return is_valid_symbol_name_v1( symbol );

                FC_ASSERT(symbol != "CDC", "Symbol must not be CDC");

                if (symbol.size() < CDC_BLOCKCHAIN_MIN_SYMBOL_SIZE)
                ...
                if (dots == 0)
                {
                    if (position > CDC_BLOCKCHAIN_MAX_SCDC_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }
                else //dots == 1 means hierarchial asset name
                {
                    if (position - dot_position - 1 > CDC_BLOCKCHAIN_MAX_SCDC_SYMBOL_SIZE)
                        FC_ASSERT(false, "Sub-symbol name too large");
                    if (symbol.size() > CDC_BLOCKCHAIN_MAX_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }

                if (symbol.find("CDC") == 0)
                    FC_ASSERT(false, "Symbol names cannot be prefixed with CDC");
        ...
        {
            // if( get_head_block_num() < CDC_V0_4_24_FORK_BLOCK_NUM )
        ...
        {
            if (imessage.size() < CDC_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
            {
                return 0;
            }
            ImessageIdType fee_coe = CDC_BLOCKCHAIN_MIN_MESSAGE_FEE_COE;
            ShareType fee = 0;
            fee = fee_coe * (imessage.size() - CDC_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
            return fee;
        }
        ShareType ChainInterface::get_asset_registration_fee(uint8_t symbol_length)const
        {
            ...
            static const ShareType long_symbol_price = 500 * CDC_BLOCKCHAIN_PRECISION; //
        ...
        double ChainInterface::to_pretty_price_double(const Price& price_to_pretty_print)const
        {
            ...
            return fc::variant(string(price_to_pretty_print.ratio * obase_asset->precision / oquote_asset->precision)).as_double() / (CDC_BLOCKCHAIN_MAX_SHARES * 1000);
        }
        ...
        uint64_t ChainInterface::get_required_confirmations()const
        {
            try {
                ...
                return CDC_BLOCKCHAIN_NUM_DELEGATES * 3;
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        {
            Asset fee(CDC_DEFAULT_TRANSACTION_FEE, 0);
            return fee;
        }
        ...
        {
            Asset fee(CDC_DEFAULT_CONTRACT_MARGIN, 0);
            return fee;
        }
        ...
        {
            return CDC_DEFAULT_TRANSACTION_EXPIRATION_SEC;
        }
        */
            ...
            const Address& to_account_address, SignedTransaction& trx)
        {
            try {
				...
				{
					FC_CAPTURE_AND_THROW(invalid_asset_symbol,("Only CDC Allowed"));
				}
