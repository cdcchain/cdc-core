...
namespace cdcchain {
    namespace client {
        ...
        const string CDC_MESSAGE_MAGIC = "Ub Signed Message:\n";
        ...
        program_options::variables_map parse_option_variables(int argc, char** argv)
        {
            ...
            {
                std::cout << fc::json::to_pretty_string(cdcchain::client::version_info()) << "\n";
                exit(0);
            }
...
#ifdef CDC_TEST_NETWORK
                        //dir_name += "-Test" + std::to_string(CDC_TEST_NETWORK_VERSION);
                        dir_name += "-Test";
#endif
                        return dir_name;
                    };

                    datadir = fc::app_path() / get_os_specific_dir_name(CDC_BLOCKCHAIN_NAME);
                }
                return datadir;
        ...
        void load_checkpoints(const fc::path& data_dir)
        {
            try {
                ...
                decltype(cdcchain::consensus::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                ...
                {
                    decltype(cdcchain::consensus::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                    if (cdcchain::consensus::detail::CHECKPOINT_BLOCKS.empty() || external_checkpoints.crbegin()->first >= cdcchain::consensus::detail::CHECKPOINT_BLOCKS.crbegin()->first)
                    {
                        ulog("Using blockchain checkpoints from file: ${x}", ("x", checkpoint_file.preferred_string()));
                        cdcchain::consensus::detail::CHECKPOINT_BLOCKS = external_checkpoints;
                        return;
                    }
                }
                ...
                if (!file_exception.valid())
                {
                    ...
                    fc::json::save_to_file(cdcchain::consensus::detail::CHECKPOINT_BLOCKS, checkpoint_file);
                }
                else
                {
                ...
                if (!cdcchain::consensus::detail::CHECKPOINT_BLOCKS.empty())
        ...
        namespace detail
        {
                ...
                const program_options::variables_map& option_variables)
            {
                if (option_variables.count("server") || option_variables.count("daemon") || cfg.rpc.enable)
                {
                    ...
                    // cdcchain::rpc::rpc_server::config rpc_config(cfg.rpc);
            ...
            void ClientImpl::start_delegate_loop()
            {
                ...
                if (!_time_discontinuity_connection.connected())
                    _time_discontinuity_connection = cdcchain::consensus::time_discontinuity_signal.connect([=](){ reschedule_delegate_loop(); });
            ...
            void ClientImpl::delegate_loop()
            {
                ...
                {
                    set_target_connections(CDC_NET_DELEGATE_DESIRED_CONNECTIONS);
                ...
                if (next_block_time.valid())
                {
                    ...
                    if (*next_block_time <= now)
                    {
                        try
                        {
                            ...
                            FC_ASSERT((now - *next_block_time) < fc::seconds(CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                ...
                uint32_t slot_number = cdcchain::consensus::get_slot_number(now);
                time_point_sec next_slot_time = cdcchain::consensus::get_slot_start_time(slot_number + 1);
                ilog("Rescheduling delegate loop for time: ${t}", ("t", next_slot_time));

                time_point scheduled_time = next_slot_time;
                if (cdcchain::consensus::ntp_time().valid())
                    scheduled_time -= cdcchain::consensus::ntp_error();
            ...
            void ClientImpl::start_rebroadcast_pending_loop()
            {
                if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
                    "rebroadcast_pending");
            }
            void ClientImpl::start_fork_update_time_loop()
            {
                if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 6)),
                    "rebroadcast_pending");
            }
            void ClientImpl::fork_update_time_loop()
            {
                ...
                {
                    cdcchain::consensus::update_ntp_time();
                    _chain_db->set_forkdb_num(fork_num);
                }
            }
            cdcchain::consensus::PublicKeyType ClientImpl::wallet_get_account_owner_publickey(const std::string& account_name)
            {
                // set limit in  simulator state
                if (_chain_db->get_is_in_simulator())
                    FC_THROW_EXCEPTION(simulator_command_forbidden, "in simulator, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::rebroadcast_pending_loop()
            {
                ...
                else
                {
                    try
                    {
                        ...
                        std::vector<cdcchain::consensus::SignedTransaction> temp_vec;
                    ...
                    try
                    {
                        ...
                        if (fork_data && fork_data->is_known)
                        {
                            if (sync_mode && !fork_data->is_linked)
                                FC_THROW_EXCEPTION(cdcchain::consensus::unlinkable_block,
                        ...
                        else
                        {
                            ...
                            if (sync_mode && !result.is_linked)
                                FC_THROW_EXCEPTION(cdcchain::consensus::unlinkable_block, "The blockchain accepted this block, but it isn't linked");
            ...
            bool ClientImpl::has_item(const cdcchain::network::ItemId& id)
            {
            ...
            */
            bool ClientImpl::handle_message(const cdcchain::network::Message& message_to_handle, bool sync_mode)
            {
                if (g_client_quit)
                    return true;
                ...
                try
                {
                    switch (message_to_handle.msg_type)
                    {
						case block_message_type:
						{
							...
							if (block_message_to_handle.block.block_size() > CDC_BLOCKCHAIN_MAX_BLOCK_SIZE)
							{
							...
							cdcchain::consensus::BlockIdType old_head_block = _chain_db->get_head_block_id();
						...
						case trx_message_type:
						{
							...
							if (trx_message_to_handle.trx.data_size() > CDC_BLOCKCHAIN_MAX_TRX_SIZE)
							{
                ...
                catch (const cdcchain::consensus::insufficient_relay_fee& original_exception)
                {
                    FC_THROW_EXCEPTION(cdcchain::network::insufficient_relay_fee, "Insufficient relay fee; do not propagate!",
                        ("original_exception", original_exception.to_detail_string()));
                }
                catch (const cdcchain::consensus::block_older_than_undo_history& original_exception)
                {
                    FC_THROW_EXCEPTION(cdcchain::network::block_older_than_undo_history, "Block is older than undo history, stop fetching blocks!",
                        ("original_exception", original_exception.to_detail_string()));
                }
            }
            ...
            */
            std::vector<cdcchain::network::ItemHashType> ClientImpl::get_item_ids(uint32_t item_type,
                const std::vector<cdcchain::network::ItemHashType>& blockchain_synopsis,
                uint32_t& remaining_item_count,
                uint32_t limit /* = 2000 */)
            {
                // limit = 20; // for testing
                FC_ASSERT(item_type == cdcchain::client::block_message_type);
                ...
                cdcchain::network::ItemHashType last_seen_block_hash;
                for (const cdcchain::network::ItemHashType& item_hash : boost::adaptors::reverse(blockchain_synopsis))
                {
                    try
                    {
                ...
                std::vector<cdcchain::network::ItemHashType> hashes_to_return;
            ...
            std::vector<cdcchain::network::ItemHashType> ClientImpl::get_blockchain_synopsis(uint32_t item_type,
            ...
            {
                FC_ASSERT(item_type == cdcchain::client::block_message_type);
                std::vector<cdcchain::network::ItemHashType> synopsis;
                uint32_t high_block_num = 0;
                uint32_t non_fork_high_block_num = 0;
                std::vector<BlockIdType> fork_history;

                if (reference_point != cdcchain::network::ItemHashType())
                {
            ...
            cdcchain::network::Message ClientImpl::get_item(const cdcchain::network::ItemId& id)
            {
                if (id.item_type == block_message_type)
                {
                    //   uint32_t block_number = _chain_db->get_block_num(id.item_hash);
                    cdcchain::client::BlockMessage block_message_to_send(_chain_db->get_block(id.item_hash));
            ...
            fc::path ClientImpl::script_compile(const fc::path& filename) const
            {
                ...
                try{
                    ...
                    {
                        FC_THROW_EXCEPTION(cdcchain::consensus::invalid_script_source_filename, "script source file name should end with .lua or .glua");
                    }
					auto p_lua_module = std::make_shared<UvmModuleByteStream>();
                    FC_ASSERT(p_lua_module, "Alloc memory for UvmModuleByteStream failed!");
					::cdcchain::consensus::contract_engine::ContractEngineBuilder builder;
                    ...
                    {
                        FC_THROW_EXCEPTION(cdcchain::consensus::save_bytecode_to_scriptfile_fail, err_msg);
                    }
            ...
            uint32_t ClientImpl::get_block_number(const cdcchain::network::ItemHashType& block_id)
            {
                return _chain_db->get_block_num(block_id);
            }

            fc::time_point_sec ClientImpl::get_block_time(const cdcchain::network::ItemHashType& block_id)
            {
                if (block_id == cdcchain::network::ItemHashType())
                {
            ...
            fc::time_point_sec ClientImpl::get_blockchain_now()
            {
                ...
                return cdcchain::consensus::nonblocking_now();
            }

            cdcchain::network::ItemHashType ClientImpl::get_head_block_id() const
            {
            ...
            {
                return cdcchain::consensus::estimate_last_known_fork_from_git_revision_timestamp(unix_timestamp);
            }
            ...
            void ClientImpl::blocks_too_old_monitor_task()
            {
                ...
                if (_chain_db->get_head_block().timestamp < cdcchain::consensus::now() - fc::seconds(CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
                    !_sync_mode &&
                ...
                if (!_blocks_too_old_monitor_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(CDC_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                    "block_monitor_task");
            }
        ...
        Client::Client(const std::string& user_agent,
            cdcchain::network::SimulatedNetworkPtr network_to_connect_to)
            : my(new detail::ClientImpl(this, user_agent))
        {
            ...
            const std::function<void(float)> replay_status_callback)
        {
            try {
                ...
                my->_wallet = std::make_shared<cdcchain::wallet::Wallet>(my->_chain_db, my->_config.wallet_enabled);
                my->_wallet->set_data_directory(wallet_dir / "wallets");

                //delete?
                //if (my->_config.mail_server_enabled)
                //{
                //my->_mail_server = std::make_shared<cdcchain::mail::server>();
                //my->_mail_server->open(data_dir / "mail");
                //}
                //my->_mail_client = std::make_shared<cdcchain::mail::client>(my->_wallet, my->_chain_db);
                ...
                if (!my->_p2p_node)
                    my->_p2p_node = std::make_shared<cdcchain::network::Node>(my->_user_agent);
        ...
        cdcchain::rpc::RpcServerPtr Client::get_rpc_server()const { return my->_rpc_server; }
        cdcchain::network::NodePtr Client::get_node()const { return my->_p2p_node; }

        fc::variant_object version_info()
        {
            string client_version(cdcchain::utilities::git_revision_description);
            const size_t pos = client_version.find('/');
            if (pos != string::npos && client_version.size() > pos)
                client_version = client_version.substr(pos + 1);
#ifdef CDC_TEST_NETWORK
            //client_version += "-testnet-" + std::to_string(CDC_TEST_NETWORK_VERSION);
            client_version += "-testnet";
#endif

            fc::mutable_variant_object info;
            info["blockchain_name"] = CDC_BLOCKCHAIN_NAME;
            info["blockchain_description"] = CDC_BLOCKCHAIN_DESCRIPTION;
            info["client_version"] = client_version;
            info["cdc_revision"] = cdcchain::utilities::git_revision_sha;
            info["cdc_revision_age"] = fc::get_approximate_relative_time_string(fc::time_point_sec(cdcchain::utilities::git_revision_unix_timestamp));
...
#if defined(CDC_CLIENT_JENKINS_BUILD_NUMBER)
            info["jenkins_build_number"] = CDC_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(CDC_CLIENT_JENKINS_BUILD_URL)
            info["jenkins_build_url"] = CDC_CLIENT_JENKINS_BUILD_URL;
#endif
        ...
        void Client::start_networking(std::function<void()> network_started_callback)
        {
            ...
            {
                cdcchain::network::ChainDownloader* chain_downloader = new cdcchain::network::ChainDownloader();
        ...
        void Client::configure_from_command_line(int argc, char** argv)
        {
            ...
            {
                my->_cli = new cdcchain::console::Cli(this, nullptr, &std::cout);
                return;
            }
            g_client = this;
            // parse command-line options
            ...
            fc::path datadir = cdcchain::client::get_data_dir(option_variables);
            fc::path walletDir = cdcchain::client::get_wallet_dir(option_variables, datadir);
            ...
            // else we use the default set in cdcchain::network::node
            ...
            else //we will accept input from the console
            {
                ...
                if (option_variables.count("log-commands") <= 0)
                {
                    ...
                    my->_cli = new cdcchain::console::Cli(this, my->_command_script_holder.get(), &std::cout);
                }
                else
                {
                    ...
                    my->_cli = new cdcchain::console::Cli(this, my->_command_script_holder.get(), my->_tee_stream.get());
            ...
            if (option_variables["upnp"].as<bool>())
            {
                ...
                my->_upnp_service = std::unique_ptr<cdcchain::network::UpnpService>(new cdcchain::network::UpnpService);
            ...
            if (option_variables.count("growl") || my->_config.growl_notify_endpoint)
            {
                ...
                std::string cdc_instance_identifier = "Cdc";
                if (option_variables.count("growl-identifier"))
                    cdc_instance_identifier = option_variables["growl-identifier"].as<std::string>();
                else if (my->_config.growl_cdc_client_identifier)
                    cdc_instance_identifier = *my->_config.growl_cdc_client_identifier;
                my->_notifier = std::make_shared<UbGntpNotifier>(host_to_notify, port_to_notify, cdc_instance_identifier, growl_password);
			...
			*/
            ...
            start_networking([=]{
                ...
                else if (!option_variables.count("disable-default-peers"))
                {
                    for (string default_peer : my->_config.default_peers)
                    {
                        ...
                        catch (cdcchain::network::endpoint_in_blacklist)
                        {
                        }
                    }
                }
            ...
            {
                my->_chain_server = std::unique_ptr<cdcchain::network::ChainServer>(
                    new cdcchain::network::ChainServer(my->_chain_db,
        ...
        cdcchain::network::NodeIdType Client::get_node_id() const
        {
        ...
        void Client::add_node(const string& remote_endpoint, int32_t oper_flag)
        {
            ...
            catch (const cdcchain::network::already_connected_to_requested_peer&)
            {
            }
        }
        ...
        {
            cdcchain::network::ItemId head_item_id;
            head_item_id.item_type = cdcchain::client::block_message_type;
            uint32_t last_block_num = my->_chain_db->get_head_block_num();
            if (last_block_num == (uint32_t)-1)
                head_item_id.item_hash = cdcchain::network::ItemHashType();
            else
                head_item_id.item_hash = my->_chain_db->get_head_block_id();
            my->_p2p_node->sync_from(head_item_id, cdcchain::consensus::get_list_of_fork_block_numbers());
        ...
        bool Client::handle_message(const cdcchain::network::Message& message, bool sync_mode)
        {
        ...
        */
        ...
        cdcchain::api::CommonApi* Client::get_impl() const
        {
            return my.get();
        }
